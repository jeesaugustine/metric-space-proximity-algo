...
class kNN_Rp:
    def __init__(self, n, k, oracle, plug_in_oracle, seed=59):
        self.n = n
        self.k = k
        self.oracle = oracle
    ...
    def _update_nha(self, i, j):
        if self.plug_in_oracle.is_uncalculated(i, j):
            dist = self.oracle(i, j)
            self.plug_in_oracle.update((i, j), dist)
        else:
            dist = self.plug_in_oracle.lookup(i, j)[0]
    ...
    def _update(self, cur, node):
        if not node:
            return
        # if len(self.NHA[node.id]) > self.k and self.plug_in_oracle.lookup(cur.id, node.id)[0] > self.PR[node.id] + (-self.NHA[node.id][0][0]):
        if len(self.NHA[node.id]) > self.k and self.plug_in_oracle.lookup(cur.id, node.id)[0] > (
                -self.NHA[node.id][0][0]):
            return
