...
class DirFeasibilityTest:
    ...
    def solve_inequality(self, c):
        ...
        if len(self.A_equality) > 0:
            return linprog(c, A_ub=self.A_inequality, b_ub=self.b_inequality, A_eq=self.A_equality, b_eq=self.b_equality, bounds=[(0.0, 1.0)], method='revised simplex')
        else:
            return linprog(c, A_ub=self.A_inequality, b_ub=self.b_inequality, bounds=[(0.0, 1.0)], method='revised simplex')
            ...
            for r in removed:
                for n in set_nodes:
                    ...
                    while i < len(candidate_set):
                        ...
                        soln_max = self.plug_in_oracle.solve_inequality(self.plug_in_oracle.eqn_maker([(r, n, -1), (candidate_set[i][0], candidate_set[i][1], 1)]))
                        #print(soln_max.fun)
                        #print(r, n, candidate_set[i][0], candidate_set[i][1])
            ...
            while len(candidate_set) > 0:
                ...
                if len(lookedup) != 0:
                    ...
                    for lkd_up in comaprision_set:
                        soln_max = self.plug_in_oracle.solve_inequality(self.plug_in_oracle.eqn_maker([(lkd_up[0], lkd_up[1], -1), (x, y, 1)]))
                        #print(soln_max.fun)
                        #print(lkd_up[0], lkd_up[1], c[0], c[1])
                        if not soln_max.success:
                            pdb.set_trace()
                        if -soln_max.fun <= 0:
...
helper_prims_plugin(order_val=order_val, scheme_chooser_list=1, scale_k=1, oracle_chooser=oracle_chooser, bounder=False, three=False)
